...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("sf_automation.log"),
...
def find_green_crawl_indicator():
    ...
    try:
        ...
        for contour in contours:
            ...
            if w > 50 and 10 < h < 30:
                logger.info(f"Found possible green indicator at x={x}, y={y}, width={w}, height={h}")
                # Draw rectangle on debug image
                debug_img = screenshot_np.copy()
                cv2.rectangle(debug_img, (x, y), (x+w, y+h), (0, 255, 0), 2)
                cv2.imwrite(os.path.join(debug_dir, f"green_indicator_{int(time.time())}.png"), 
    ...
    try:
        ...
        if dialog_win:
            ...
            try:
                ...
                dialog_screenshot = pyautogui.screenshot(region=(
                    dialog_win.left, dialog_win.top, 
                    dialog_win.width, dialog_win.height
                ))
                
                # Save this for debugging
                dialog_screenshot.save(os.path.join(debug_dir, f"dialog_window_{int(time.time())}.png"))
...
def find_and_click_export_button():
    ...
    try:
        ...
        if sf_win:
            ...
            export_y = sf_win.top + 70   # Adjusted to match the Export button in your image
            pyautogui.click(export_x, export_y)
            logger.info(f"Clicked estimated Export button position at {export_x}, {export_y}")
            ...
            try:
                ...
                for contour in contours:
                    ...
                    if 50 < w < 150 and 20 < h < 50:
                        logger.info(f"Found possible green Save button at x={x}, y={y}, width={w}, height={h}")
                        # Click the center of the button
                        center_x = x + (w // 2)
                        center_y = y + (h // 2)
                        pyautogui.click(center_x, center_y)
                        logger.info(f"Clicked green button at x={center_x}, y={center_y}")
...
def is_application_ready(window):
    ...
    try:
        ...
        # Save this for debugging
        screenshot.save(os.path.join(debug_dir, f"app_readiness_check_{int(time.time())}.png"))
...
def wait_for_complete_crawl_then_export():
    ...
    while time.time() - start_time < max_wait_time:
        try:
            ...
            status_width = sf_win.width // 3  # Capture about 1/3 of the width where status text is
            
            status_screenshot = ImageGrab.grab(bbox=(
                left_x, bottom_y - 20, 
                left_x + status_width, bottom_y + 20
            ))
            ...
            progress_screenshot = ImageGrab.grab(bbox=(
                sf_win.left, top_bar_y - 10,
                sf_win.right, top_bar_y + 10
            ))
            
            progress_path = os.path.join(debug_dir, f"progress_bar_{int(time.time())}.png")
            ...
            if green_width > 0.99:
                logger.info("✅ Crawl appears to be 100% complete based on progress bar!")
            ...
            bottom_status = ImageGrab.grab(bbox=(
                sf_win.right - 300, sf_win.bottom - 30,
                sf_win.right, sf_win.bottom
            ))
            
            bottom_status_path = os.path.join(debug_dir, f"bottom_status_{int(time.time())}.png")
            bottom_status.save(bottom_status_path)
            
            # Visual check to see if the status text shows 100% or "0 Remaining"
            # Since OCR would require additional libraries, we'll just check if
            # a human looking at the debug image can verify completion
            
            logger.info(f"Waiting for crawl to finish... Check debug images if available.")
...
def export_after_completion():
    ...
    try:
        ...
        for _ in range(4):  # Skip past File, View, Mode, Configuration to reach Bulk Export
    ...
    try:
        ...
        bulk_export_x = sf_win.left + 242  # Approximate X position based on your screenshot
...
def main():
    ...
    if not sf_win:
        logger.error("❌ Couldn't find the Screaming Frog window—make sure it's installed and running.")
    ...
    while ready_attempts < 5:  # Try up to 5 times
        ...
        logger.info(f"Application not ready yet, waiting... (attempt {ready_attempts+1}/5)")
        time.sleep(3)
        ready_attempts += 1
        ...
        sf_win = find_window(possible_titles)
        if not sf_win:
            logger.error("❌ Lost window handle while waiting for application to be ready")
    ...
    monitor_thread = Thread(target=monitor_for_crawl_completion, args=(stop_monitoring,))
        ...
        for attempt in range(3):  # Try up to 3 times
            ...
            if find_and_click_export_button():
                ...
                try:
                    ...
                    internal_option_y = pyautogui.position().y + 100  # Offset down to find Internal option
                    ...
                    if save_dialog:
                        ...
                        # Handle the save dialog
                        if handle_save_dialog(filename="internal_all.csv", directory=downloads_dir):
